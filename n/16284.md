---

title: 'Geotiff error when trying to export'
tagnames: 'balloon-mapping, kite-mapping, mapknitter, aerial-photography, software, leaflet, upgrades, stitching, question:general, question:mapknitter, updates, leaflet-distortable-image, map-stitching, garden-mapping, gardens, question:gis, response:15568, geotiff'
author: Zemans
nid: 16284
uid: 532957
path: /notes/Zemans/05-03-2018/geotiff-error-when-trying-to-export
---

<p>Redirecting to: <a href='/notes/Zemans/05-03-2018/geotiff-error-when-trying-to-export'>Geotiff error when trying to export</a></p><script>window.location.replace('/notes/Zemans/05-03-2018/geotiff-error-when-trying-to-export');</script>