---

title: 'Water quality parameters'
tagnames: 'water-quality-monitoring, water-quality, conductivity, list:plots-waterquality, open-water, ph, parent:water-quality, nitrogen, redirect:common-water-contaminants'
author: vjpixel
nid: 12477
uid: 420617
path: /wiki/water-quality-parameters
---

<p>Redirecting to: <a href='/wiki/water-quality-parameters'>Water quality parameters</a></p><script>window.location.replace('/wiki/water-quality-parameters');</script>