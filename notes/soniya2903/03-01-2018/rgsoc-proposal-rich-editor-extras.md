---
nid: 15846
title: RGSoC proposal: Rich Editor Extras
path: public/static/notes/soniya2903/03-01-2018/rgsoc-proposal-rich-editor-extras.md
uid: 526441
tagnames: software,gsoc,soc,response:13975,gsoc-2018,soc-2018,rgsoc-2018,soc-2018-proposals
---

# RGSoC proposal: Rich Editor Extras

(This template is for students applying to summer code programs with Public Lab. Use this link to start writing a post: [https://publiclab.org/post?n=899&tags=soc,soc-2018,soc-2018-proposals](https://publiclab.org/post?n=899&tags=soc,soc-2018,soc-2018-proposals) You can delete this line once you've started filling it out.)

---------

## About me

_Names: Soniya Vyas & Shital Mule_

**Affiliation :- Rajarambapu Institute of Technology, Islampur, Maharashtra**

**Location: Sangli**

**Email : [soniyavyas29@gmail.com](mailto:soniyavyas29@gmail.com), [shitalmule04@gmail.com](mailto:shitalmule04@gmail.com)**

**We both are perceiving B.Tech in computer science.**

## Project description

Rich Editor Extras require better user interfaces for various inline power tags. This will need Bootstrap, HTML UI work on the PublicLab. Also the project needs to be integrated with [PublicLab.org](http://PublicLab.org). website backend.

### Abstract/summary (<20 words):

Create better User Interfaces for inserting inline power tags.

### Problem:-

There is need of better User Interfaces, since if we have a look at the applications today, they have multiple features endorsed in it. Hence to compete with these many applications , there has to be awesome interface with as many as possible features. It avoids the inconvenience of the user as well who will use it. And also last but not least, for a good Editor the user interface with many features is the real key.

### Timeline/milestones

Week1(July 1 to July 8):-

Understanding the code base

Week 2(July 9 to July 15):-

Study inline power tags

Week 3, Week 4, Week 5(July 16 to August 5):-

Designing of UI

Testing

Implementation of inline power tags like Activity grids, Question grids, maps

Testing

Week 6, Week 7(August 6 to August 19):-

Implementation of Inline Maps, Inline Graphs, Prompts

Week 8, Week 9(August 20 to September 2):-

Integration with [PublicLab.org](http://PublicLab.org). website backend

Week 10(September 3 to September 10):-

Review of the work done so far

Week 11(September 11 to September 17):

Final Testing

Week 12(September 18 to September 24):-

Submission of project

Needs

_The only resources required will be online documentation, materials. Mentors will be required the most._

### First-time contribution

We still don't have any contribution to the site, as of now. Still we will try to contribute before the project starts.

### Experience

We both have sound knowledge of Python, Java. We are still beginners in Ruby on Rails.

Link :-

1\. [https://github.com/son-vyas/Dating-Website](https://github.com/son-vyas/Dating-Website)

2\. [https://github.com/son-vyas/Color](https://github.com/son-vyas/Color)

3\. [https://github.com/son-vyas/Matching-Game](https://github.com/son-vyas/Matching-Game)

4\. [https://github.com/shitalmule04/OpenSourceOS-Website](https://github.com/shitalmule04/OpenSourceOS-Website)

5\. [https://github.com/shitalmule04/python-learning](https://github.com/shitalmule04/python-learning)

6\. [https://github.com/shitalmule04/WebBrowser](https://github.com/shitalmule04/WebBrowser)

---------

### Teamwork

We have not work together. If this opportunity is given to us then this will be our first chance to be as a team.

---------

### Passion

_Public Lab is platform for the environmetal concerns. By this platform it desires to change the attitude of people to look towards environmental, social, etc. Hence , this give us more enthusiasm to get along with Public Lab._

### Audience

Our work will help the society since the ultimate motivation is to change the way people look towards the world in environmental, social, political terms.

### Commitment

_Given this chance to us, we will make sure each day we spent come out with fruitful result and at the end the desired outcome is achieved._