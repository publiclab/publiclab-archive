---
title: "A Catalogue of Civic Open Hardware Projects"

tagnames: 'open-hardware'
author: bsugar
path: /wiki/a-catalogue-of-civic-open-hardware-projects.md
nid: 10710
uid: 217812
cids: 
---

# A Catalogue of Civic Open Hardware Projects

by [tomasdiez](/profile/tomasdiez), [jalbertbowdenii](/profile/jalbertbowdenii), [amysoyka](/profile/amysoyka), [bsugar](/profile/bsugar), [msaunby](/profile/msaunby), [oliblondeau](/profile/oliblondeau), [warren](/profile/warren), [danbeavers](/profile/danbeavers)

July 19, 2014 19:25 | Tags: [open-hardware](/tag/open-hardware)

----

###Beyond Modifying Your Honda Civic    

Civic open hardware projects are surprisingly difficult to find.  Google results are littered with ways to modify your Honda Civic.  As a proud owner of a 2000 Honda Civic with just over 108,000 miles, I appreciate this.  That said, it's not quite what we're looking for.    

The phrase "civic open hardware" hasn't become as common place as "open gov" or "open data", and the phrase "civic hacking" has been passively defined to mean tools that primarily use software.    
<br>

###Defining Civic Open Source Hardware  

####Civic  

The word "civic" is a pretty all encompassing term;  indeed, a fun challenge is to find something in our lives in which civics to does not play a role.  The definition need not be limited to things we normally associate with government. One guiding principle might be that it should involve community and participation.  

There has been a fair amount written about defining "civic hacking" (links to articles in the future.  For this page, the goal is to have a collection of projects that make use of "open hardware".  
<br>

####Open      

The phrase "open source' is the topic of more debate than one would expect, with many nuances and forks.  A collection of some general definitions can be found in the links below:  

http://en.wikipedia.org/wiki/Open_source_hardware
http://www.oshwa.org/definition/  

While it is hoped that the list below will elucidate a number of open source projects, there is now a space below for closed, or seemingly closed projects, too.

<br>

####Hardware  

As for the definition of "hardware", there is an opportunity here to reassert a definition that is not limited to computation.  Artifacts such as D-Lab's Bicilavadora, a pedal powered washing machine, certainly employ wares that are hard, and can even be picked up at a hardware store.  
<br>

###The List  

Insert your link here, and re-organize as you see fit!

1. Obviously: [Public Lab Tools](http://publiclab.org/tools)
2. [John Keefe](http://johnkeefe.net/), [Team Blinky](http://teamblinky.tumblr.com/), [Cicada Tracker](http://project.wnyc.org/cicadas/)
3. [Many Labs](https://www.manylabs.org/)
4. [Low-Cost Illuminated Threat Alert](http://www.instructables.com/id/Low-Cost-Illuminated-Signage-for-Todays-Responsibl/)
5. [The EyeWriter](http://www.instructables.com/id/The-EyeWriter/)
6. [Between the Bars](https://betweenthebars.org) a paper based blogging platform for those who are incarcerated.
7. [Timenesia](http://www.timenesia.org)
8. [D-Lab](http://d-lab.mit.edu/resources)
9. [Environmental Sensing and Visualization](http://dm.lmc.gatech.edu/research/labs/public-design-workshop/environmental-sensing-and-visualization/), [Public Design Workshop](http://dm.lmc.gatech.edu/research/labs/public-design-workshop/)
10. [I, Citizen](http://icitizen2000.org/), The Activists Board Game, [Museum of Contemporary Phenomena](http://mcphenomena.us/)
11. Mesh Networking, e.g. [Byzantium](http://project-byzantium.org/about/), [Commotion](https://commotionwireless.net/)
12. [DiY Bio Printer](http://www.instructables.com/id/DIY-BioPrinter/)
13. [Particulate Air Pollution Sensor](http://www.instructables.com/id/How-to-measure-particulate-air-pollution-and-share/)
14. [Sound Generating Protest Flag](http://www.instructables.com/id/Sound-Generating-Protest-Flag/)
15. [Overpass Light Brigade](http://overpasslightbrigade.org)
16. [Pedal Powered Generator](http://pedalpower2thepeople.pbworks.com/w/page/47830717/FrontPage)
17. [Smart Citizen Project](http://www.smartcitizen.me)
18. [Open Source Beehives](http://www.opensourcebeehives.net/)
19. [Global Village Construction Set](http://opensourceecology.org/gvcs/)
20. [Safe Cast](http://safecast.org)
21. [Citizenwatt.paris](http://wiki.citizenwatt.paris/doku.php?id=en:start)
22. [OpenROV Open-source underwater robots for exploration and education](http://openrov.com/)
23. [AirPi Air quality sensor shield for Raspberry Pi](http://airpi.es/)
24. [openMaterials](http://openmaterials.org/)
25. [Open Source Civic EV Kit](http://civic-ev.blogspot.com/)

<br>

Not, or seemingly not open, but inspiring nonetheless:

1. [Rainforest Connection](https://rfcx.org/)