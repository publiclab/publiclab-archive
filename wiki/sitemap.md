---
title: sitemap
tagnames: 
author: liz
path: /wiki/sitemap.md
nid: 17203
uid: 7

---

# sitemap

by [liz](../profile/liz)

October 01, 2018 21:44 | Tags: 

Pages in **bold** indicate **sysadmin-only editing access**

## Connect online
* https://publiclab.org/signup -- _
* https://publiclab.org/profile/ -- _your profile page once you sign up. You can add tags such as interest or location, edit your bio, and upload a profile image or avatar._ 
* https://publiclab.org/subscriptions -- _where you can control what tags (AKA topics) you are subscribed to, and adjust your email notifications_
* https://publiclab.org/lists -- _needs updating_
* https://publiclab.org/forum -- _the Community Science Forum_
* https://publiclab.org/newsletter -- _monthly newsletter composed by staff, easy to understand_
* FOR RSS READERS ONLY **https://publiclab.org/feed.rss** 

## Connect in-person
* https://publiclab.org/events -- _has a Google calendar, and describes types of events_
* https://publiclab.org/barnraising -- _our major multi-day, retreat-style in-person event_

## Activity by geography
* https://publiclab.org/region
* https://publiclab.org/places

## Overview
* https://publiclab.org/blog
* https://publiclab.org/stories


## Who we are
The Public Lab community: 

* **https://publiclab.org/people**
* https://publiclab.org/partners
* https://publiclab.org/organizers

The Public Lab community is backed by a non-profit to focus impact on environmental health justice, all the following links pertain to the nonprofit:  

* https://publiclab.org/about
* https://publiclab.org/plots-staff
* https://publiclab.org/donate
vhttps://publiclab.org/how-we-are-funded
* https://publiclab.org/board
* https://publiclab.org/nonprofit-values
* _https://publiclab.org/nonprofit-initiatives -- _deprecated, but still linked from the [About page](https://publiclab.org/about)_

## How we do it
* https://publiclab.org/conduct
* https://publiclab.org/barnstars -- _style_
* https://publiclab.org/facilitation
* https://publiclab.org/moderation
* https://publiclab.org/licenses + stub https://publiclab.org/attribution
* https://publiclab.org/community-development
* https://publiclab.org/glossary
* https://publiclab.org/evaluation

## Getting started
* https://publiclab.org/welcome
* https://publiclab.org/getting-started
* https://publiclab.org/new-projects

## Diving deeper
* https://start-enviro-monitor-study

## Further reading
* https://publiclab.org/media -- _articles about Public Lab_
* http://publiclab.org/annual-reports -- _published by the nonprofit_
* https://publiclab.org/suggested-readings
* https://publiclab.org/ethics-and-politics

## Statistics about community activity
* **https://publiclab.org/stats**
* **https://publiclab.org/stats/subscriptions**
* **https://publiclab.org/contributors**
* **https://publiclab.org/users/role/...** -- _experimental feature_

## Website: knowledge base organization
The Public Lab website has two main types of content: 

* **https://publiclab.org/wiki** -- _multi-author content, editable by anyone._
* **https://publiclab.org/notes** -- _single-author content, also can add co-authors. Content only editable by authors_

Navigating the Public Lab website: 

* **https://publiclab.org/dashboard** -- _redirects from publiclab.org when you are logged-in to show you a custom view of content. Redirects from publiclab.org/research_
* **https://publiclab.org/search** -- _
* **https://publiclab.org/tags**
* **https://publiclab.org/tag/* -- _view of all content tagged with a certain tag `*`_
* UNDER CONSTRUCTION **https://publiclab.org/map** - _the map of all geographically tagged content on publiclab.org (not actual maps)_

## Website: knowledge base pages controlled by tags + an editable feature at top
* **https://publiclab.org/questions**
* **https://publiclab.org/methods** -- _also see https://publiclab.org/wiki/methods, described below under `Website: how to write, format, and organize content`_

## Website: how to write, format, and organize content
* https://publiclab.org/wiki/authoring-help
* https://publiclab.org/power-tags
* https://publiclab.org/wiki-templates -- _this page contains much of what could be described as "the Public Lab model" and should be highlighted more_
* https://publiclab.org/wiki/methods -- _excellent guides for posting new methods_

https://publiclab.org/kits -- _the Kits Initiative, and home of Community Kits_

## Software for processing environmental data, and data archives
* **https://publiclab.org/archive** -- _all curated map data_
* **https://mapknitter.org** -- _where you can process aerial image data into maps_
* **https://spectralworkbench.org** _where you can process spectral data into calibrated spectra_
* **https://infragram.org** _where you can process infrared data into NDVI or other_
* _need to include additional / other sites where data is processed and/or stored_




